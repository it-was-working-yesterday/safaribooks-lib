...
class Display:
    ...
    def unhandled_exception(self, _, o, tb):
        ...
        self.fatal("Unhandled Exception: %s (type: %s)" % (o, o.__class__.__name__))
...
class SafariBooks:
    ...
    def __init__(self, args):
        ...
        if not args.cred:
            if not os.path.isfile(COOKIES_FILE):
                self.display.fatal("Login: unable to find `cookies.json` file.\n"
    ...
    def do_login(self, email, password):
        ...
        if response == 0:
            self.display.fatal("Login: unable to reach Safari Books Online. Try again...")
        ...
        try:
            ...
            self.display.fatal("Login: unable to complete login on Safari Books Online. Try again...")
        ...
        if response == 0:
            self.display.fatal("Login: unable to perform auth to Safari Books Online.\n    Try again...")
    ...
    def check_login(self):
        ...
        if response == 0:
            ...
            self.display.fatal("Authentication issue: unable to access profile page.")

        elif "user_type\":\"Expired" in response.text:
            self.display.fatal("Authentication issue: account subscription expired.")
    ...
    def get_book_info(self):
        ...
        if response == 0:
            self.display.fatal("API: unable to retrieve book info.")

        response = response.json()
        if not isinstance(response, dict) or len(response.keys()) == 1:
            self.display.fatal(self.display.api_error(response))
    ...
    def get_book_chapters(self, page=1):
        ...
        if response == 0:
            self.display.fatal("API: unable to retrieve book chapters.")
    ...
    def parse_html(self, root, first_page=False):
        if random() > 0.8:
            if len(root.xpath("//div[@class='controls']/a/text()")):
                self.display.fatal(self.display.api_error(" "))
...
SafariBookArgs = namedtuple("SafariBookArgs", ['bookid', 'kindle', 'cred', 'no_cookies', 'log'])


def download_book(bookid: str, kindle: bool = False, cred: str = None, no_cookies: bool = False,
                  log: bool = False):
    args = SafariBookArgs(bookid, kindle, cred, no_cookies, log)
    return SafariBooks(args)
